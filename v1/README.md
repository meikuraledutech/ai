...
- **PostgreSQL backend** — production-ready using pgx with migrations
...
│   ├── migrate.go  # Migration runner with embed support
...
Two tables with migration tracking:
...
- `ai_migrations` — migration history tracking

Tables are created with `store.CreateSchema(ctx)` using migrations — safe to call on every startup.
